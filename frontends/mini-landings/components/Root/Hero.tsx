...
export default function Hero({ loading }: HeroProps) {
  ...
  const router = useRouter(); // новая строка
  ...
  const handleJoinClick = () => { // новая функция
    router.push('/new');
  };
    ...
    <div className="main-container-hero">
      <div className="container-hero container-text-button-hero">
        ...
        <div className="sub-title">
          <button 
            onClick={handleJoinClick} // новый обработчик
            className={`px-20 py-6 text-4xl font-bold text-rose-400 border border-rose-600 hover:bg-rose-600 hover:text-white focus:outline-none focus:ring active:bg-rose-500 ${!loading && subTitleAnimation ? 'animate-sub-title' : 'initial-hidden'}`}
          >
            Присоединиться
          </button>
        </div>
